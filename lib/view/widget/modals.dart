...
import 'package:kas_mini_flutter_app/model/report_payment_model.dart';
...
import 'package:kas_mini_flutter_app/view/widget/date_from_to/from_to_v3.dart';
import 'dart:io';
  ...
  static Future<void> saveExcelExpenseData(
    ...
    if (fileName.isEmpty) {
      ...
      for (var data in expenseData) {
        ...
        sheet.cell(CellIndex.indexByString('D$rowIndex')).value = formattedDateAdded;
      ...
      for (TransactionData transaction in transactions) {
        ...
        for (int j = 0; j < products.length; j++) {
          if (j == 0) {
            ...
            sheet.cell(CellIndex.indexByString('F$rowIndex')).value = transaction.transactionTotal;
            sheet.cell(CellIndex.indexByString('G$rowIndex')).value = transaction.transactionDiscount;
            sheet.cell(CellIndex.indexByString('H$rowIndex')).value = transaction.transactionPayAmount;
            sheet.cell(CellIndex.indexByString('I$rowIndex')).value = transaction.transactionProfit;
          }
          rowIndex++;
        }
      }
